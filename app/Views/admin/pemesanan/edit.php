...
<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
    ...
    <div class="ms-auto">
        <a href="/admin/pemesanan/detail/<?= $pemesanan['idpesan'] ?>" class="btn btn-secondary">
...
<div class="card">
    <div class="card-body">
        <div class="card-title d-flex align-items-center">
            ...
            <h5 class="mb-0 text-primary">Form Edit Pemesanan</h5>
        </div>
        <hr>
        ...
        <form action="<?= base_url('admin/pemesanan/update/' . $pemesanan['idpesan']) ?>" method="POST" id="bookingForm">
            <?= csrf_field() ?>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="pelanggan_name" name="pelanggan_name"
                                placeholder="Pilih pelanggan" readonly
                                value="<?= $selected_user['name'] ?? old('pelanggan_name') ?>">
                            <input type="hidden" id="iduser" name="iduser"
                                value="<?= $pemesanan['iduser'] ?? old('iduser') ?>">
                            <button class="btn btn-primary" type="button" id="pilihPelangganBtn">
                                <i class="bx bx-search"></i> Pilih
                            </button>
                        </div>
                        <?php if (isset($validation['iduser'])): ?>
                            <div class="text-danger"><?= $validation['iduser'] ?></div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="paket_name" name="paket_name"
                                placeholder="Pilih paket wisata" readonly
                                value="<?= $selected_paket['namapaket'] ?? old('paket_name') ?>">
                            <input type="hidden" id="idpaket" name="idpaket"
                                value="<?= $pemesanan['idpaket'] ?? old('idpaket') ?>">
                            <input type="hidden" id="paket_harga" name="paket_harga"
                                value="<?= $selected_paket['harga'] ?? old('paket_harga') ?>">
                            <input type="hidden" id="paket_durasi" name="paket_durasi"
                                value="<?= $selected_paket['durasi'] ?? old('paket_durasi') ?>">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="tgl_berangkat" name="tgl_berangkat"
                            value="<?= $pemesanan['tgl_berangkat'] ?? old('tgl_berangkat') ?>"
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control" id="tgl_selesai" name="tgl_selesai"
                            value="<?= $pemesanan['tgl_selesai'] ?? old('tgl_selesai') ?>" readonly>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="jumlah_peserta" name="jumlah_peserta"
                            min="1" value="<?= $pemesanan['jumlah_peserta'] ?? old('jumlah_peserta') ?>" required>
                    ...
                    <!-- <div class="mb-3">
                        ...
                        <textarea class="form-control" id="catatan" name="catatan" rows="3"><?= $pemesanan['catatan'] ?? old('catatan') ?></textarea>
                    </div> -->
                </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="form-select">
                            <option value="pending" <?= ($pemesanan['status'] ?? old('status')) == 'pending' ? 'selected' : '' ?>>Pending (Menunggu Pembayaran)</option>
                            <option value="down_payment" <?= ($pemesanan['status'] ?? old('status')) == 'down_payment' ? 'selected' : '' ?>>DP Terbayar</option>
                            <option value="waiting_confirmation" <?= ($pemesanan['status'] ?? old('status')) == 'waiting_confirmation' ? 'selected' : '' ?>>Menunggu Konfirmasi</option>
                            <option value="confirmed" <?= ($pemesanan['status'] ?? old('status')) == 'confirmed' ? 'selected' : '' ?>>Confirmed</option>
                            <option value="cancelled" <?= ($pemesanan['status'] ?? old('status')) == 'cancelled' ? 'selected' : '' ?>>Cancelled</option>
                        </select>
                    </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="ms-3">
                                    ...
                                    <div id="paket-info" class="text-secondary mb-2">
                                        <?php if (isset($selected_paket['namapaket']) && isset($selected_paket['harga']) && isset($selected_paket['durasi'])): ?>
                                            <strong><?= $selected_paket['namapaket'] ?></strong><br>
                                            Durasi: <?= $selected_paket['durasi'] ?> hari<br>
                                            Harga: Rp <?= number_format($selected_paket['harga'], 0, ',', '.') ?>
                                        <?php else: ?>
                                            Silakan pilih paket wisata
                                        <?php endif; ?>
                                    </div>
                                    <div class="mt-2 d-flex justify-content-between align-items-center">
                                        <strong>Total Biaya:</strong>
                                        <span id="total-biaya" class="fs-5 text-primary fw-bold">
                                            Rp <?= number_format($pemesanan['totalbiaya'] ?? 0, 0, ',', '.') ?>
                                        </span>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="d-md-flex align-items-center mt-3">
                        <div class="ms-auto">
                            <a href="/admin/pemesanan/detail/<?= $pemesanan['idpesan'] ?>" class="btn btn-light px-4">Batal</a>
